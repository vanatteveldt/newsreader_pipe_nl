...
function parse {
    java -jar $MDIR/ixa-pipe-tok/target/ixa-pipe-tok-1.8.5-exec.jar tok -l nl  |\
	$MDIR/morphosyntactic_parser_nl/run_parser.sh  |\
	cat
}

function annotate {
    java -jar $MDIR/ixa-pipe-nerc/target/ixa-pipe-nerc-1.6.1-exec.jar tag -m $TDIR/nerc-models-1.5.4/nl/nl-6-class-clusters-sonar.bin |\
	...
	java -Xmx1812m -cp "$MDIR/KafSaxParser/target/KafSaxParser-v3.1.1-jar-with-dependencies.jar" eu.kyotoproject.util.FixDutchSrl |\
